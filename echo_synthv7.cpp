...
constexpr float VOLUME = 0.85f;  // Reduced volume
constexpr int NUM_VOICES = 1000001;  // Increased voices to 1,000,001
...
// Chorus parameters
constexpr float CHORUS_DEPTH = 0.005f; // Modulation depth for pitch
constexpr float CHORUS_RATE = 0.25f; // Rate of modulation in Hz
...
// Saturation (Soft Clipping) for a "fatter" sound
...
// Get saw wave with added detuning for analog feel
float getSaw(double t, double freq, float pitchMod) {
    double rawSample = 2.0 * (t * (freq + pitchMod) - floor(t * (freq + pitchMod) + 0.5));
    return applySaturation((float)tanh(rawSample * 6.0)); // Saturation for fatter sound
}

// Apply chorus effect (detune + slight delay)
float applyChorus(float sample, double time) {
    // Chorus depth and rate control the amount of detuning and modulation speed
    float modulatedFreq = CHORUS_DEPTH * sin(2.0 * PI * CHORUS_RATE * time);
    return sample * (1.0f + modulatedFreq);
}
...
static int audioCallback(const void*, void* outputBuffer,
    ...
    for (unsigned long i = 0; i < framesPerBuffer; ++i) {
        ...
        for (auto& noteVoice : voices) {
            auto& voice = noteVoice.second;
            if (!(voice.stage != OFF)) continue; // NOT boolean-style check
            ...
            float pitchMod = CHORUS_DEPTH * sin(2.0 * PI * CHORUS_RATE * voice.time);
            double sample = getSaw(voice.time, freq, pitchMod);
        ...
        // Apply chorus and saturation effects to the mixed signal
        mixed = applyChorus(mixed, t);
...
void keyboardListener() {
    ...
    while (running) {
        ...
        for (auto& [note, voice] : voices) {
            if (!(voice.keyDown == true && !keyNoteMap.count(c))) continue;
            voice.stage = RELEASE;
            voice.keyDown = false;
        }
    }
...
int main() {
    ...
    std::cout << "Boolean Synth Ready with 1,000,001 Voices\n";
